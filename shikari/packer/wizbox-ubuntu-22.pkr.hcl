...
variable "source_image" {
  ...
  default = {
    ...
    "checksum" = "file:https://cloud-images.ubuntu.com/releases/jammy/release/SHA256SUMS"
  }
}
...
locals {
  ...
  image_id_string = "wizbox-${var.source_image["distro"]}-22"
}
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      "sudo apt clean",
      "sudo apt update",
      "sudo apt install -y unzip wget traceroute",

      # For multicast DNS to use with socket_vmnet in Lima we use systemd-resolved.
      "sudo apt install -y crudini chrony && sudo systemctl enable --now systemd-resolved",
      "sudo mkdir /etc/systemd/resolved.conf.d/",
      "curl -LO https://raw.githubusercontent.com/pixelb/crudini/refs/heads/master/crudini.py",
      "sudo python3 crudini.py --ini-options=nospace --set /etc/systemd/resolved.conf.d/mdns.conf Resolve MulticastDNS yes",
